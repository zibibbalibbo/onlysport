...
var HomeManager = {
    ...
    createItems: function (items) {
        ...
        var htmlObjs = [];
        $(items).each((i, el) => {
            var $item = $('<div class="grid-item" id="thumb_' + el.id + '"></div>');
    ...
    loadElements: function () {
        SocialManager.loadGoogleActivities(this.nextPageToken)
            .done((resp) => {
                ...
                if (this.isFirst) {
                    this.isFirst = false;
                    let sharedId = sharer.getId();
                    if (!!sharedId) {
                        this.higlightItem(sharedId);
                    }
                }
            });
    },
    higlightItem: function (id) {
        this.$wallWrapper.find('#' + id).click();
    }
}
